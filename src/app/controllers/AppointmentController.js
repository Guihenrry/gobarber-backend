...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (isBefore(hourStart, new Date())) {
      return res.status(400).json({ error: 'Past date a not permitted' });
    }
    ...
    if (checkAvailability) {
      return res
        .status(400)
        .json({ error: 'Appointment date is note availability' });
    }
  ...
  async delete(req, res) {
    ...
    if (!appointment) {
      return res.status(400).json({ error: 'Appointment not exists' });
    }
